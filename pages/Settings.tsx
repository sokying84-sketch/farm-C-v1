...
import { Save, Database, Code, CheckCircle2, Copy, AlertCircle, Link, ExternalLink, FileSpreadsheet, RefreshCw, UploadCloud, DownloadCloud, Radio, Lock } from 'lucide-react';
import { getAppSettings, saveAppSettings, pushFullDatabase, pullFullDatabase } from '../services/sheetService';
...
const SettingsPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'connection' | 'schema' | 'code' | 'sync'>('connection');
  ...
  useEffect(() => {
    ...
    setIsHardcoded(!!settings.isHardcoded);
  }, []);

  const handleSave = () => {
    if (isHardcoded) return; // Prevent save if hardcoded
            ...
            <div className="space-y-6 max-w-2xl">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full rounded-lg border-slate-300 border p-3 focus:ring-2 focus:ring-nature-500 font-mono text-sm ${isHardcoded ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : ''}`}
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      className={`flex-1 rounded-lg border-slate-300 border p-3 focus:ring-2 focus:ring-nature-500 font-mono text-sm ${isHardcoded ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : ''}`}
                    />
                    {sheetUrl && (
                      <a 
                        href={sheetUrl} 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center space-x-3 bg-slate-50 p-4 rounded-lg border border-slate-200">
                    ...
                    <button
                      onClick={() => !isHardcoded && setUseMock(!useMock)}
                      disabled={isHardcoded}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        useMock ? 'bg-nature-500' : 'bg-slate-300'
                      } ${isHardcoded ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
